...
*** I don't love the cofiguration leakage
    ...
    3. A valid OpenSSH public key must be registered in your GitHub account,
       which must match the key used to perform the 'git clone'
       action. Setting this up requires that you paste the text of the public
       key into a text box of the GitHub web UI, on the page:
